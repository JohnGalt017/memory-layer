...
describe("ReadFile UseCase", () => {
  ...
  test("should call loadFileAtLevel when level is L0", async () => {
    vi.spyOn(fileRepositoryStub, "loadFileAtLevel").mockResolvedValue("abstract text");
    const params: ReadFileParams = {
      projectName: "project-1",
      fileName: "file1.md",
      level: "L0",
    ...
    expect(fileRepositoryStub.loadFileAtLevel).toHaveBeenCalledWith("project-1", "file1.md", "L0");
    expect(result).toBe("abstract text");
  });

  test("should call loadFileAtLevel when level is L1", async () => {
    vi.spyOn(fileRepositoryStub, "loadFileAtLevel").mockResolvedValue("overview text");
    const params: ReadFileParams = {
      projectName: "project-1",
      fileName: "file1.md",
      level: "L1",
    ...
    expect(fileRepositoryStub.loadFileAtLevel).toHaveBeenCalledWith("project-1", "file1.md", "L1");
  });

  test("should use loadFile when level is omitted (backward compat)", async () => {
    const loadFileSpy = vi.spyOn(fileRepositoryStub, "loadFile");
    const params: ReadFileParams = {
      projectName: "project-1",
      fileName: "file1.md",
  ...
  test("should use loadFile when level is L2 (backward compat)", async () => {
    const loadFileSpy = vi.spyOn(fileRepositoryStub, "loadFile");
    const params: ReadFileParams = {
      projectName: "project-1",
      fileName: "file1.md",
