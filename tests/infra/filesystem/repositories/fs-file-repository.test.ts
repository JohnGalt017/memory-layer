...
describe("FsFileRepository", () => {
  ...
  describe("loadFileAtLevel", () => {
    it("should return null when file does not exist", async () => {
      const result = await repository.loadFileAtLevel("proj", "nonexistent.md", "L2");
      expect(result).toBeNull();
    });

    it("L2 should return full content", async () => {
      await repository.writeFile(projectName, "test.md", "# Title\n\nFull content.");
      const result = await repository.loadFileAtLevel(projectName, "test.md", "L2");
      expect(result).toContain("Full content.");
    });

    it("L0 should return abstract line format", async () => {
      const content = "---\ntype: architecture\nstatus: active\n---\n\n# Title\n\nAbstract text.";
      await repository.upsertFile(projectName, "arch.md", content);
      const result = await repository.loadFileAtLevel(projectName, "arch.md", "L0");
      expect(result).toContain("arch.md");
      expect(result).toContain("[architecture]");
      expect(result).toContain("(active)");
    });

    it("L1 should return frontmatter + first 50 lines", async () => {
      const content = "---\ntype: notes\n---\n\nFirst line.\nSecond line.";
      await repository.upsertFile(projectName, "notes.md", content);
      const result = await repository.loadFileAtLevel(projectName, "notes.md", "L1");
      expect(result).toContain("type: notes");
  ...
  describe("listFilesWithMetadata", () => {
    it("should return empty array when project has no files", async () => {
      const result = await repository.listFilesWithMetadata("emptyproj");
      expect(result).toEqual([]);
    });

    it("should return metadata for files in a project", async () => {
      await repository.upsertFile(projectName, "file.md", "---\ntype: notes\n---\n\nContent here.");
      const result = await repository.listFilesWithMetadata(projectName);
    ...
    it("should include all projects when projectName is omitted", async () => {
      await repository.upsertFile("proj1", "a.md", "Content A.");
      await repository.upsertFile("proj2", "b.md", "Content B.");
      const result = await repository.listFilesWithMetadata();
